...
ystart = 4
datap = ["malware", "phishing", "syskill", "webkb"]
for dp in datap:
    plt.text(0, ystart, dp, ha="left", family='sans-serif', size=11)
    add_global(ax, ystart, 0.2)
    add_protocol(ax, ystart, ystart)
    add_path(ax, ystart, ystart)
    add_params(ax, ystart, ystart)
    add_domain(ax, ystart, ystart)
    add_subdomain(ax, ystart, ystart)
    add_tld(ax, ystart, ystart)
    add_file(ax, ystart, ystart)
    ystart += 2
...
def main(files, prefix, fnames, imports):
    ...
    for f in files:
        ...
        plotables = get_plotable_features(f, prefix)
...
if __name__ == '__main__':
    ...
    my_parser.add_argument('feature_names',
